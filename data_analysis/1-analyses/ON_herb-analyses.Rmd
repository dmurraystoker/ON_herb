...
```{r Site Map, include = FALSE, fig.cap = "Map of sampling locations, with sites coloured by city. Urban sites are indicated by circles and rural sites by triangles. Toronto was sampled along an urbanization gradient while the 5 Cities were sampled using urban-rural pairs."}
...
species.traits <- read.csv("data/species_dispersal_traits.csv")
```
...
```{r Toronto Abundance ANCOVA: Check Assumptions, include = FALSE, fig.cap = "Diagnostic plots of the Toronto abundance ANCOVA. The model had slight but acceptable over-dispersion (dispersion statistic = 1.25)"}
...
```{r Toronto Richness ANCOVA: Check Assumptions, include = FALSE, fig.cap = "Diagnostic plots of the Toronto richness ANCOVA. The model has acceptable dispersion (dispersion statistics = 0.44)."}
...
```{r Toronto Diversity ANCOVA: Check Assumptions, include = FALSE, fig.cap = "Diagnostic plots of the Toronto diversity ANCOVA."}
...
```{r 5 Cities Abundance ANOVA: Check Assumptions, include = FALSE, fig.cap = "Diagnostic plots of the 5 Cities abundance ANOVA. The model has good dispersion (dispersion statistic = 0.93)."}
...
```{r 5 Cities Richness ANOVA: Check Assumptions, include = FALSE, fig.cap = "Diagnostic plots of the 5 Cities richness ANOVA. The "}
...
```{r 5 Cities Diversity ANOVA: Check Assumptions, include = FALSE, fig.cap = "Diagnostic plots of the 5 Cities diversity ANOVA."}
...
where city, habitat, season, and all two- and three-way interactions were fitted as fixed effects and population was fitted as a random intercept. The Toronto model with a random slope and intercept had better fit compared to the model with only a random intercept (random slope and intercept: $AIC_c = 629.515$, $R^2_{marginal} = 0.645$, $R^2_{conditional} = 0.787$; random intercept only: $AIC_c = 660.430$, $R^2_{marginal} = 0.639$, $R^2_{conditional} = 0.756$), but the 5 Cities random slope and intercept model failed to converge Therefore. we fitted the 5 Cities model with just a random intercept for population.
...
```{r Toronto Leaf Herbivory ANOVA: Check Assumptions, include = FALSE, fig.cap = "Diagnostic plots of the Toronto leaf herbivory linear mixed-effects model."}
...
```{r 5 Cities Leaf Herbivory ANOVA: Check Assumptions, include = FALSE, fig.cap = "Diagnostic plots of the 5 Cities leaf herbivory linear mixed-effects model."}
...
We evaluated whether community composition and individual herbivore species varied with urbanization by using multivariate generalized linear models. In this approach, separate GLMs with shared predictor variables are fitted using `manyglm()` and then resampling methods are used to test which variables or factors are associated with the multivariate abundances. With this approach, it is possible to answer: (1) do communities vary in composition with urbanization and (2) which herbivore species are influenced by urbanization?
...
five.cities.community.matrix <- five.cities.herbivores[, 7:15] %>%
	as.data.frame()
```
...
```{r Toronto Herbivore Community: Check manyglm Assumptions, include = FALSE, fig.cap = "Diagnostic plot of the Toronto herbivore community manyglm model."}
...
```{r 5 Cities Herbivore Community: Check manyglm Assumptions, include = FALSE, fig.cap = "Diagnostic plot of the 5 Cities herbivore community manyglm model."}
...
\newpage
# Dispersal-Urbanization Relationships

We evaluated whether abundances of different dispersal traits varied with urbanization using a similar approach to the multivariate GLMs for community composition and herbivore species. Instead of supplying a site-by-species abundance matrix as the response, we calculated the abundances of each trait for each site and provided a site-by-trait-abundance matrix as the response. With this modification, we can test (1) whether dispersal trait abundances vary with urbanization and (2) which specific dispersal traits are influenced by urbanization?

We fitted the Toronto and 5 Cities `manyglm()` models exactly like the community composition and herbivore species models, except for the site-by-trait-abundance matrix as the response. Both models were fitted using GLMs with a negative-binomial error distribution, and model assumptions were evaluated graphically. The influence of main effects and interactions in the model was determined using likelihood ratio tests and resampling, resulting in a multivariate test for the overall abundance matrix and univariate tests for each herbivore species. We used 999 resampling iterations, with P-values adjusted for multiple testing through an automated step-down resampling procedure.


\newpage
```{r Trait Abundance Function, echo = TRUE}
## Function to calculate trait abundances for each site
trait_by_population_abundance <- function(j) {
	## Load required packages
	require(dplyr)
	
	## Set trait matrix
	trait.matrix <- species.traits
	
	## Sequentially calculate abundances for each trait for all sites in the supplied taxa matrix
	dispersal.1000_km  <- j * trait.matrix[, 2] [match(names(j), trait.matrix$Species)][col(j)]
	dispersal.100_km   <- j * trait.matrix[, 3] [match(names(j), trait.matrix$Species)][col(j)]
	dispersal.30_40_km <- j * trait.matrix[, 4] [match(names(j), trait.matrix$Species)][col(j)]
	dispersal.10_15_km <- j * trait.matrix[, 5] [match(names(j), trait.matrix$Species)][col(j)]
	dispersal.1_2_km   <- j * trait.matrix[, 6] [match(names(j), trait.matrix$Species)][col(j)]
	
	## Set list of traits; remove UID column
	trait.list <- list(
		dispersal.1000_km[, -1], dispersal.100_km[, -1], dispersal.30_40_km,
		dispersal.10_15_km[, -1], dispersal.1_2_km[, -1]
	)
	...
	colnames(trait.abundances.bin.1)[1:5] <- colnames(trait.matrix)[2:6]
	
	## Set final trait abundance tibble
	trait.abundances <- trait.abundances.bin.1 %>%
		as_tibble()
}
```
...
```{r Calculate Trait Abundances, echo = TRUE}
## Calculate Toronto trait abundances
toronto.trait.abundances <- trait_by_population_abundance(
	toronto.community.matrix
	)

## Calculate 5 Cities trait abundances
five.cities.trait.abundances <- trait_by_population_abundance(
	five.cities.community.matrix
	)
```
...
```{r Toronto Dispersal Traits: manyglm Fitting, echo = TRUE}
## Fit the Toronto herbivore community model
toronto.dispersal.traits.model <- manyglm(
	mvabund(toronto.trait.abundances) ~ Distance * Season,
	data = toronto.herbivores,
	family = "negative_binomial"
	)
```
...
```{r Toronto Dispersal Traits: Check manyglm Assumptions, include = FALSE, fig.cap = "Diagnostic plot of the Toronto dispersal traits manyglm model."}
plot(toronto.dispersal.traits.model)
```

\vspace{10pt}

```{r Toronto Dispersal Traits: manyglm ANOVA, echo = TRUE}
## ANOVA on the Toronto herbivore community manyglm
toronto.dispersal.traits.anova <- anova(
	toronto.dispersal.traits.model,
	nBoot = 999,
	p.uni = "adjusted"
	)
```
...
```{r Toronto Dispersal Traits: ANOVA Summary, echo = FALSE}
kable(
	toronto.dispersal.traits.anova$table,
	booktabs = TRUE,
	digits = 3,
	caption = "Overall summary of the Toronto dispersal traits manyglm.",
	col.names = c("Residual df", "df", "Test Statistic", "P-value")
	) %>% 
	kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r Toronto Dispersal Traits: Individual Test Statistics, echo = FALSE}
kable(
	toronto.dispersal.traits.anova$uni.test,
	booktabs = TRUE,
	digits = 3,
	caption = "Test statistics for each species in the Toronto dispersal traits manyglm."
	) %>% 
	kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r Toronto Dispersal Traits: Individual P-Values, echo = FALSE}
kable(
	toronto.dispersal.traits.anova$uni.p,
	booktabs = TRUE,
	digits = 3,
	caption = "P-values associated with the test statistics for each species in the Toronto dispersal traits manyglm."
	) %>% 
	kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r 5 Cities Dispersal Traits: manyglm Fitting, echo = TRUE}
## Fit the 5 Cities herbivore community model
five.cities.dispersal.traits.model <- manyglm(
	mvabund(five.cities.trait.abundances) ~ City * Habitat * Season,
	data = five.cities.herbivores,
	family = "negative_binomial"
	)
```
...
```{r 5 Cities Dispersal Traits: Check manyglm Assumptions, include = FALSE, fig.cap = "Diagnostic plot of the 5 Cities dispersal traits manyglm model."}
plot(five.cities.dispersal.traits.model)
```

\vspace{10pt}

```{r 5 Cities Dispersal Traits: manyglm ANOVA, echo = TRUE}
## ANOVA on the 5 Cities herbivore community manyglm
five.cities.dispersal.traits.anova <- anova(
	five.cities.dispersal.traits.model,
	nBoot = 999,
	p.uni = "adjusted"
	)
```
...
```{r 5 Cities Dispersal Traits: ANOVA Summary, echo = FALSE}
kable(
	five.cities.dispersal.traits.anova$table,
	booktabs = TRUE,
	digits = 3,
	caption = "Overall summary of the 5 Cities dispersal traits manyglm.",
	col.names = c("Residual df", "df", "Test Statistic", "P-value")
	) %>% 
	kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r 5 Cities Dispersal Traits: Individual Test Statistics, echo = FALSE}
kable(
	five.cities.dispersal.traits.anova$uni.test,
	booktabs = TRUE,
	digits = 3,
	caption = "Test statistics for each species in the 5 Cities dispersal traits manyglm."
	) %>% 
	kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r 5 Cities Dispersal Traits: Individual P-Values, echo = FALSE}
kable(
	five.cities.dispersal.traits.anova$uni.p,
	booktabs = TRUE,
	digits = 3,
	caption = "P-values associated with the test statistics for each species in the 5 Cities dispersal traits manyglm."
	) %>% 
	kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
\newpage
## Urbanization (pSEM 1) | Presence-Absence
...
\newpage
## Urbanization & Herbivore Interactions (Hypothesized pSEM 3) |  Presence-Absence
...
\newpage
## pSEM Comparisons (Presence-Absence)

| Model      | Fisher's *C*| df   | P    | $AIC_c$    | $\Delta AIC_c$ | K    |
|:----------:|------------:|-----:|------:|----------:|---------------:|-----:|
| pSEM 2     | 0           | 8    | 0.999 | 87.100    | 0              | 26   |
| pSEM 1     | 62.180      | 90   | 0.989 | 148.827   | 85.102         | 20   |
| pSEM 3     | 0           | 8    | 0.999 | 219.760   | 132.681        | 41   |
...
Estimated marginal means and contrasts were analyzed using the `emmeans` package. Predictions were weighted by cell frequency, and P-values were adjusted using the false discovery rate (FDR).
...
```{r Toronto Abundance ANCOVA Marginal Means, echo = TRUE, results = "hide"}
## emmeans
# Season
...
```{r Toronto Richness ANCOVA Marginal Means, echo = TRUE, results = "hide"}
## emmeans
# Season
...
```{r Toronto Diversity ANCOVA Marginal Means, echo = TRUE, results = "hide"}
## emmeans
# Season
...
```{r 5 Cities Abundance ANOVA Marginal Means: Main Effects, echo = TRUE, results = "hide"}
## emmeans
# City
...
```{r 5 Cities Abundance ANOVA Marginal Means: Two-Way Interactions, echo = TRUE, results = "hide"}
## emmeans
# City x Habitat
...
```{r 5 Cities Abundance ANOVA Marginal Means: Three-Way Interaction, echo = TRUE, results = "hide"}
## emmeans
...
```{r 5 Cities Richness ANOVA Marginal Means: Main Effects, echo = TRUE, results = "hide"}
## emmeans
# City
...
```{r 5 Cities Richness ANOVA Marginal Means: Two-Way Interactions, echo = TRUE, results = "hide"}
## emmeans
# City x Habitat
...
```{r 5 Cities Richness ANOVA Marginal Means: Three-Way Interaction, echo = TRUE, results = "hide"}
## emmeans
# City x Habitat x Season
...
```{r 5 Cities Diversity ANOVA Marginal Means: Main Effects, echo = TRUE, results = "hide"}
## emmeans
# City
...
```{r 5 Cities Diversity ANOVA Marginal Means: Two-Way Interactions, echo = TRUE, results = "hide"}
## emmeans
# City x Habitat
...
```{r 5 Cities Diversity ANOVA Marginal Means: Three-Way Interaction, echo = TRUE, results = "hide"}
## emmeans
# City x Habitat x Season
...
```{r Toronto Leaf Herbivory ANCOVA Marginal Means, echo = TRUE, results = "hide"}
## emmeans
# Season
...
```{r 5 Cities Leaf Herbivory ANOVA Marginal Means: Main Effects, echo = TRUE, results = "hide"}
## emmeans
# City
...
```{r 5 Cities Leaf Herbivory ANOVA Marginal Means: Two-Way Interactions, echo = TRUE, results = "hide"}
## emmeans
# City x Habitat
...
```{r 5 Cities Leaf Herbivory ANOVA Marginal Means: Three-Way Interaction, echo = TRUE, results = "hide"}
## emmeans
# City x Habitat x Season
